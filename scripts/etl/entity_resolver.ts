...
function main() {
  ...
  function findInPlayersDb(
    ...
    if (exact) {
      ...
      if (nationality) {
        const byNat = exact.filter(
          (p) =>
            normalizeName(p.nationality) === normalizeName(nationality)
        );
      ...
      if (position) {
        ...
        const byPos = exact.filter((p) => {
          ...
          return (
            pPos === posNorm ||
            pPos.includes(posNorm) ||
            posNorm.includes(pPos)
          );
        });
    ...
    if (ln.length >= 4) {
      let lastNameCandidates = pdbByLastName.get(ln);
  ...
  console.log(
    `  Resolved: ${careerToPdb.size} / ${careerPaths.length}`
  );
  ...
  console.log(
    `  Resolved: ${transferToPdb.size} / ${transfers.length}`
  );
  ...
  console.log(
    `  Unique names: ${allMatchNames.size}`
  );
  console.log(
    `  Resolved: ${matchNameResolved.size} / ${allMatchNames.size}`
  );
  ...
  writeFileSync(
    resolve(ROOT, "data/player_id_map.json"),
    JSON.stringify(playerIdMap, null, 2)
  );
  writeFileSync(
    resolve(ROOT, "data/club_id_map.json"),
    JSON.stringify(clubIdMap, null, 2)
  );
  ...
  if (unresolvedCareer.length > 0) {
    ...
    if (unresolvedCareer.length > 20)
      console.log(`  ... and ${unresolvedCareer.length - 20} more`);
  }
  ...
  if (unresolvedMatchNames.length > 0) {
    ...
    for (const n of unresolvedMatchNames.sort().slice(0, 20))
      console.log(`  - ${n}`);
